...
func registerGRPCGateway(ctx context.Context, mux *http.ServeMux, grpcAddr string) error {
	...
	err := randomcoffee.RegisterRandomCoffeeHandlerFromEndpoint(ctx, gwMux, grpcAddr, opts)
	if err != nil {
		return fmt.Errorf("can't register random coffee handler for grpc endpoint: %w", err)
	}
