...
class OracleService(oracle_pb2_grpc.OracleServicer):
    ...
    def RetrainLFMv1(self, request, context):
        ...
        data['interests__from'] = data['interests__from'].apply(lambda y: [str(x) + '_interest__from' for x in y])

        data['interests__to'] = data['interests__to'].apply(lambda y: [str(x) + '_interest__to' for x in y])
        ...
        for id, (_, row) in zip(data_with_unique_from['from'], data_with_unique_from[from_feature_names].iterrows()):
            l = []
            for feat in row:
                l.append(feat)
